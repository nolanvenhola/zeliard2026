...
class ViewModule:
    def __init__(self, parent, render_callback):
        ...
        tk.Label(self.frame, text="[보기]", font=('Malgun Gothic', 9, 'bold')).pack(side=tk.LEFT, padx=5)

        tk.Label(self.frame, text="모드:").pack(side=tk.LEFT)
        ...
        tk.Label(self.frame, text="가로:").pack(side=tk.LEFT)
        ...
        tk.Label(self.frame, text="확대:").pack(side=tk.LEFT, padx=2)
        ...
        tk.Checkbutton(self.frame, text="격자", variable=self.show_grid, command=self.render_callback).pack(side=tk.LEFT, padx=5)
