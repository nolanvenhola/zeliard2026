...
#include "dialogs/BulkDecompileDialog.h"
...
void MainWindow::on_actionExportDecompiledCode_triggered()
{
    auto decompilers = Core()->getDecompilers();
    if (decompilers.isEmpty()) {
        QMessageBox::warning(this, tr("Export Decompiled Code"),
                             tr("No decompiler is available. "
                                "Install a decompiler plugin such as rz-ghidra."));
        return;
    }
    ...
    if (functions.isEmpty()) {
        QMessageBox::warning(this, tr("Export Decompiled Code"),
                             tr("No functions found. Analyze the binary first."));
        return;
    }

    QString selectedDecompilerId = Config()->getSelectedDecompiler();
    Decompiler *decompiler = Core()->getDecompilerById(selectedDecompilerId);
    if (!decompiler) {
        decompiler = decompilers.first();
    }

    QFileDialog dialog(this, tr("Export Decompiled Code"));
    dialog.setAcceptMode(QFileDialog::AcceptSave);
    dialog.setFileMode(QFileDialog::AnyFile);
    dialog.setNameFilter(tr("C source file (*.c);;Text file (*.txt);;All files (*)"));
    dialog.selectFile("decompiled");
    dialog.setDefaultSuffix("c");
    if (!dialog.exec()) {
        return;
    }

    QString outputPath = dialog.selectedFiles().first();
    auto *exportDialog = new BulkDecompileDialog(decompiler, functions, outputPath, this);
