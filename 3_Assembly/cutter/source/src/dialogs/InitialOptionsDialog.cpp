      ...
      core(Core())
{
    ...
    // Default to x86 / 16-bit for DOS binary analysis.
    int x86Idx = ui->archComboBox->findText("x86");
    if (x86Idx >= 0) {
        ui->archComboBox->setCurrentIndex(x86Idx);
    }
    ui->bitsComboBox->setCurrentText("16");
